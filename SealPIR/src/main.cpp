...
//   int N = enc_params.poly_modulus_degree();
  int logN = get_power_of_two(N);
     ...
     // get the sum of elements in pir_params.nvec
     uint64_t sum = 0;
     for (int i = 0; i < pir_params.nvec.size(); i++) {
       sum += pir_params.nvec[i];
     }

  for (int i = 0; i < ceil(log2(sum)); i++) {
    galois_elts.push_back(1 + N / exponentiate_uint(2, i));
  }     
  ...
  GaloisKeys galois_keys; 
     ...
     galois_keys.load(context, strstream);
